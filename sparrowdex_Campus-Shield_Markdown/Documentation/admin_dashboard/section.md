...
<document-code-reference section="Admin Dashboard Component">
{"files": [
  {
...
The Admin Dashboard component is a critical part of the Campus-Shield system, providing administrators with a comprehensive overview of campus safety reports and statistics. It is implemented in React and integrates closely with the backend API to fetch and display data.

<artifact ArtifactUUID="ca6d836a-bdaf-4d28-afc5-1925d9f1b151">Dashboard Statistics and Data Fetching</artifact>

The component fetches dashboard data on mount using the `fetchDashboardData` function:
...
This function makes parallel requests to fetch statistics and active chat data, utilizing the browser's `fetch` API and handling authentication via JWT tokens.

### Stats Display

The dashboard displays key statistics in a grid layout:
...
{stats && (
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    {/* Similar structures for other stats */}
  </div>
)}
```

<artifact ArtifactUUID="1bc95a4f-610d-4dc2-9ef3-f9a6d4f14aa7">Report Management and Filtering</artifact>

The component implements report filtering functionality:
...
This allows administrators to quickly focus on specific types of reports based on status, priority, and category.

### Report Details Modal

When an admin selects a report, a detailed modal is displayed:
...
This modal provides in-depth information about the selected report and allows for status updates.

<artifact>Backend Integration for Admin Dashboard</artifact>

The backend route `/api/admin/stats` provides the statistics displayed on the dashboard:
...
router.get('/stats', auth, admin, async (req, res) => {
  try {
    if (isMongoConnected()) {
      ...
      // Use memory store for stats if DB is not connected
    }
  } catch (error) {
...
This route is protected by authentication and admin middleware to ensure only authorized users can access the data.

## Security and Access Control

<document-code-reference section="Security and Access Control">
{"files": [
  {
...
The Admin Dashboard is secured through multiple layers:

1. Route-level protection using the `auth` and `admin` middleware.
2. Client-side checks using the `useAuth` hook to verify user role.
3. JWT token-based authentication for API requests.

The `admin` middleware on the server ensures that only users with the admin role can access sensitive routes:
...
<document-code-reference section="Responsive Design">
{"files": [
  {
    "name": "Campus-Shield/client/src/pages/AdminDashboard.tsx",
    "description": "Implementation file",
...
The Admin Dashboard implements a responsive design to ensure usability across different device sizes:
